...
describe("grid matching", () => {
  it("matches shaped patterns position-independently", () => {
    const planks = { item: "flatcraft:item:oak_planks", count: 1 };
  ...
  it("matches the stick column (1x2 pattern) anywhere", () => {
    const planks = { item: "flatcraft:item:oak_planks", count: 1 };
    expect(matchGrid(grid({ 2: planks, 5: planks }), recipes(), 3)?.id).toBe("flatcraft:item:stick");
    expect(matchGrid(grid({ 3: planks, 6: planks }), recipes(), 3)?.id).toBe("flatcraft:item:stick");
  ...
  it("matches shapeless recipes and respects grid size limits", () => {
    const log = { item: "flatcraft:item:oak_log", count: 3 };
    expect(matchGrid(grid({ 5: log }), recipes(), 2)?.id).toBe("flatcraft:item:oak_planks");
    // A 3x3-only recipe is unavailable in the 2x2 inventory grid.
    const planks = { item: "flatcraft:item:oak_planks", count: 1 };
    const stick = { item: "flatcraft:item:stick", count: 1 };
    ...
    expect(matchGrid(pickaxeGrid, recipes(), 3)?.id).toBe("flatcraft:item:wooden_pickaxe");
    expect(matchGrid(pickaxeGrid, recipes(), 2)).toBeNull();
  });

  it("rejects patterns with wrong extra items", () => {
    const planks = { item: "flatcraft:item:oak_planks", count: 1 };
    const dirt = { item: "flatcraft:item:dirt", count: 1 };
...
describe("cursor click mechanics", () => {
  it("left click picks up, places, merges and swaps", () => {
    expect(clickStack(null, { item: "flatcraft:item:dirt", count: 5 }, "left")).toEqual({
      cursor: { item: "flatcraft:item:dirt", count: 5 },
      slot: null,
    });
    expect(clickStack({ item: "flatcraft:item:dirt", count: 5 }, null, "left")).toEqual({
      cursor: null,
      slot: { item: "flatcraft:item:dirt", count: 5 },
    });
    expect(clickStack({ item: "flatcraft:item:dirt", count: 5 }, { item: "flatcraft:item:dirt", count: 62 }, "left")).toEqual({
      cursor: { item: "flatcraft:item:dirt", count: 3 },
      slot: { item: "flatcraft:item:dirt", count: 64 },
    });
    expect(clickStack({ item: "flatcraft:item:dirt", count: 5 }, { item: "flatcraft:item:sand", count: 2 }, "left")).toEqual({
      cursor: { item: "flatcraft:item:sand", count: 2 },
      slot: { item: "flatcraft:item:dirt", count: 5 },
    });
  });

  it("right click takes half and places one", () => {
    expect(clickStack(null, { item: "flatcraft:item:dirt", count: 5 }, "right")).toEqual({
      cursor: { item: "flatcraft:item:dirt", count: 3 },
      slot: { item: "flatcraft:item:dirt", count: 2 },
    });
    expect(clickStack({ item: "flatcraft:item:dirt", count: 5 }, null, "right")).toEqual({
      cursor: { item: "flatcraft:item:dirt", count: 4 },
      slot: { item: "flatcraft:item:dirt", count: 1 },
    });
  });
});
...
describe("grid crafting via commands", () => {
  it("crafts through the 2x2 grid with cursor clicks", () => {
    ...
    state.inventory[0] = { item: "flatcraft:item:oak_log", count: 1 };
    ...
    expect(state.cursor).toEqual({ item: "flatcraft:item:oak_planks", count: 4 });
    ...
    expect(countInInventory(state.inventory, "flatcraft:item:oak_planks")).toBe(4);
  });
  ...
  it("blocks 3x3 grid cells and recipes without a crafting table", () => {
    ...
    state.cursor = { item: "flatcraft:item:dirt", count: 1 };
    sim.tick([
      { player, command: { type: "slot_click", slot: { container: "craft_grid", index: 8 }, button: "left" } },
    ]);
    expect(state.craftGrid[8]).toEqual({ item: "flatcraft:item:dirt", count: 1 });
  });

  it("consumes one item per grid cell when taking the result", () => {
    const sim = new Simulation(SEED);
    const { player, state } = joinPlayer(sim);
    state.craftGrid[0] = { item: "flatcraft:item:oak_planks", count: 3 };
    state.craftGrid[3] = { item: "flatcraft:item:oak_planks", count: 1 };
    sim.tick([{ player, command: { type: "slot_click", slot: { container: "craft_result" }, button: "left" } }]);
    expect(state.cursor).toEqual({ item: "flatcraft:item:stick", count: 4 });
    expect(state.craftGrid[0]).toEqual({ item: "flatcraft:item:oak_planks", count: 2 });
...
describe("furnace", () => {
  ...
  it("smelts iron ore into ingots while burning coal", () => {
    ...
    state.cursor = { item: "flatcraft:item:iron_ore", count: 2 };
    sim.tick([
    ...
    expect(furnace.output).toEqual({ item: "flatcraft:item:iron_ingot", count: 2 });
    ...
    expect(state.cursor).toEqual({ item: "flatcraft:item:iron_ingot", count: 2 });
  });
  ...
  it("rejects non-fuel in the fuel slot and keeps output take-only", () => {
    ...
    expect(state.cursor).toEqual({ item: "flatcraft:item:dirt", count: 1 });
  });
  ...
  it("does not burn fuel with nothing to cook", () => {
    ...
    const furnace = sim.furnaces.values().next().value!;
    expect(furnace.fuel).toEqual({ item: "flatcraft:item:coal", count: 1 });
    expect(furnace.burnLeft).toBe(0);
  });

  it("hands its contents to the miner when broken", () => {
    const { sim, player, state, fx, fy } = furnaceSetup();
    state.cursor = { item: "flatcraft:item:iron_ore", count: 3 };
    sim.tick([
      { player, command: { type: "slot_click", slot: { container: "furnace", x: fx, y: fy, slot: "input" }, button: "left" } },
    ]);
    state.inventory[0] = { item: "flatcraft:item:iron_pickaxe", count: 1 };
    ...
    expect(countInInventory(state.inventory, "flatcraft:item:iron_ore")).toBe(3);
    state.x = fx + 0.5;
    for (let i = 0; i < 30; i++) sim.tick([]);
    expect(countInInventory(state.inventory, "flatcraft:item:furnace")).toBe(1);
  ...
  it("smelting recipes exist for ores, sand and cobblestone", () => {
    expect(RECIPES.get("flatcraft:item:iron_ingot")?.kind).toBe("smelting");
    expect(RECIPES.get("flatcraft:item:gold_ingot")?.kind).toBe("smelting");
    expect(RECIPES.get("flatcraft:item:glass")?.kind).toBe("smelting");
    expect(RECIPES.get("flatcraft:item:stone")?.kind).toBe("smelting");
  });

  it("tool tier chain: stone pick -> iron ore -> iron pick -> diamond", () => {
    expect(canHarvest(BlockId.IronOre, { item: "flatcraft:item:stone_pickaxe", count: 1 })).toBe(true);
    expect(canHarvest(BlockId.DiamondOre, { item: "flatcraft:item:iron_pickaxe", count: 1 })).toBe(true);
    expect(canHarvest(BlockId.DiamondOre, { item: "flatcraft:item:golden_pickaxe", count: 1 })).toBe(false);
    expect(canHarvest(BlockId.Stone, { item: "flatcraft:item:diamond_pickaxe", count: 1 })).toBe(true);
  });
});
