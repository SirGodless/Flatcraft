...
describe("villager trading", () => {
  it("loads trades from JSON with resolvable items", () => {
    ...
    expect(TRADES.some((t) => t.result.item === "elytra")).toBe(true);
  });
  ...
  it("exchanges cost for result through a villager", () => {
    ...
    const villager = sim.spawnMob("villager", state.x + 1.5, state.y, out);
    const coalTrade = TRADES.findIndex((t) => t.cost.item === "coal");
    state.inventory[0] = { item: "coal", count: 20 };

    sim.tick([{ player, command: { type: "trade", entity: villager.id, trade: coalTrade } }]);
    expect(countInInventory(state.inventory, "emerald")).toBe(1);
    expect(countInInventory(state.inventory, "coal")).toBe(8);
  });
  ...
  it("rejects trades without the items, out of range, or on non-villagers", () => {
    ...
    const pig = sim.spawnMob("pig", state.x + 1, state.y, out);
    ...
    const far = sim.spawnMob("villager", state.x + 30, state.y, out);
