...
describe("recipe data", () => {
  ...
  it("classifies grid sizes like Minecraft", () => {
    expect(RECIPES.get("flatcraft:item:oak_planks")!.gridSize).toBe(2); // 1 ingredient
    expect(RECIPES.get("flatcraft:item:stick")!.gridSize).toBe(2); // 1x2 pattern
    expect(RECIPES.get("flatcraft:item:crafting_table")!.gridSize).toBe(2); // 2x2 pattern
    expect(RECIPES.get("flatcraft:item:wooden_pickaxe")!.gridSize).toBe(3); // 3 wide
    expect(RECIPES.get("flatcraft:item:wooden_sword")!.gridSize).toBe(3); // 3 tall
  });

  it("counts pattern symbols as ingredient totals", () => {
    const pickaxe = RECIPES.get("flatcraft:item:wooden_pickaxe")!;
    // Plank ingredients are the "#planks" tag: any wood type works.
    expect(pickaxe.ingredients.get("#planks")).toBe(3);
    expect(pickaxe.ingredients.get("flatcraft:item:stick")).toBe(2);
    expect(ingredientOptions("#planks")).toContain("flatcraft:item:oak_planks");
    expect(ingredientOptions("#planks")).toContain("flatcraft:item:birch_planks");
    expect(ingredientOptions("#planks")).toContain("flatcraft:item:spruce_planks");
  });
});
...
describe("inventory", () => {
  it("stacks up to 64 and overflows into new slots", () => {
    ...
    expect(addToInventory(slots, "flatcraft:item:dirt", 70)).toBe(0);
    expect(slots[0]).toEqual({ item: "flatcraft:item:dirt", count: 64 });
    expect(slots[1]).toEqual({ item: "flatcraft:item:dirt", count: 6 });
    expect(countInInventory(slots, "flatcraft:item:dirt")).toBe(70);
  });

  it("reports overflow when completely full", () => {
    const slots = createInventory();
    const leftover = addToInventory(slots, "flatcraft:item:dirt", 64 * INVENTORY_SIZE + 5);
    expect(leftover).toBe(5);
  });

  it("removal is all-or-nothing", () => {
    const slots = createInventory();
    addToInventory(slots, "flatcraft:item:stick", 10);
    expect(removeFromInventory(slots, "flatcraft:item:stick", 11)).toBe(false);
    expect(countInInventory(slots, "flatcraft:item:stick")).toBe(10);
    expect(removeFromInventory(slots, "flatcraft:item:stick", 10)).toBe(true);
    expect(countInInventory(slots, "flatcraft:item:stick")).toBe(0);
  });

  it("respects maxStack 1 for tools", () => {
    const slots = createInventory();
    addToInventory(slots, "flatcraft:item:wooden_pickaxe", 2);
    expect(slots[0]).toEqual({ item: "flatcraft:item:wooden_pickaxe", count: 1 });
    expect(slots[1]).toEqual({ item: "flatcraft:item:wooden_pickaxe", count: 1 });
  });
});
...
describe("crafting via commands", () => {
  it("crafts planks from logs (2x2, no table needed)", () => {
    ...
    give(state, "flatcraft:item:oak_log", 2);
    sim.tick([{ player, command: { type: "craft", recipe: "flatcraft:item:oak_planks" } }]);
    expect(countInInventory(state.inventory, "flatcraft:item:oak_planks")).toBe(4);
    expect(countInInventory(state.inventory, "flatcraft:item:oak_log")).toBe(1);
  });

  it("rejects crafting with missing ingredients", () => {
    const sim = new Simulation(SEED);
    const { player } = joinPlayer(sim);
    const result = sim.tick([{ player, command: { type: "craft", recipe: "flatcraft:item:stick" } }]);
  ...
  it("requires a crafting table for 3x3 recipes", () => {
    const sim = new Simulation(SEED);
    const { player, state } = joinPlayer(sim);
    give(state, "flatcraft:item:oak_planks", 3);
    give(state, "flatcraft:item:stick", 2);

    const denied = sim.tick([{ player, command: { type: "craft", recipe: "flatcraft:item:wooden_pickaxe" } }]);
    ...
    sim.tick([{ player, command: { type: "craft", recipe: "flatcraft:item:wooden_pickaxe" } }]);
    expect(countInInventory(state.inventory, "flatcraft:item:wooden_pickaxe")).toBe(1);
    expect(countInInventory(state.inventory, "flatcraft:item:oak_planks")).toBe(0);
    expect(countInInventory(state.inventory, "flatcraft:item:stick")).toBe(0);
  });
});
...
describe("drops and placement", () => {
  it("mining a block with the right tool yields its drop into the inventory", () => {
    ...
    expect(countInInventory(state.inventory, "flatcraft:item:stone")).toBe(0);
  });

  it("block drops follow the registry (grass->dirt, coal_ore->coal, leaves->nothing)", () => {
    expect(blockDrops(BlockId.Grass)).toEqual({ item: "flatcraft:item:dirt", count: 1 });
    expect(blockDrops(BlockId.CoalOre)).toEqual({ item: "flatcraft:item:coal", count: 1 });
    expect(blockDrops(BlockId.OakLeaves)).toBeNull();
    expect(blockDrops(BlockId.Dirt)).toEqual({ item: "flatcraft:item:dirt", count: 1 });
  });

  it("placing uses the selected hotbar stack and consumes it", () => {
    const sim = new Simulation(SEED);
    const { player, state } = joinPlayer(sim);
    give(state, "flatcraft:item:dirt", 2);
    const x = Math.floor(state.x) + 1;
    ...
    expect(result).toContainEqual({
      event: { type: "block_changed", dim: "flatcraft:dimension:overworld", x, y, block: BlockId.Dirt },
    });
    expect(sim.world.getBlock(x, y)).toBe(BlockId.Dirt);
    expect(countInInventory(state.inventory, "flatcraft:item:dirt")).toBe(1);
  });
  ...
  it("rejects placing with an empty hand or a non-block item", () => {
    ...
    give(state, "flatcraft:item:stick", 1);
